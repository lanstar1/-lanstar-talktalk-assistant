...
test("해상도 문의는 연결 불량 대신 해상도와 배율 점검 중심으로 답변한다", () => {
  const engine = new ReplyEngine({
    examples: [
      {
        ...
        source: "상품Q&A",
        productName: "랜스타 LS-UH319-W USB3.0 to HDMI 영상 컨버터",
        customerText: "해상도가 너무 낮아요. 800x600으로만 잡힙니다.",
        answerText: "디스플레이 설정과 해상도, 배율 설정을 확인 부탁드립니다.",
        orderSummary: ""
      }
    ],
    policies,
    productCatalog: {
      findBestMatch() {
        return {
          ...
          matchType: "exact",
          category: "컨버터",
          formFactor: "USB 3.0 to HDMI 컨버터",
          material: "ABS",
          usage: "모니터 출력",
          specs: {
            INPUT: "USB 3.0",
          ...
          specsText: "INPUT: USB 3.0 / OUTPUT: HDMI",
          summary: "LS-UH319-W 컨버터"
        };
      }
    }
  });
  ...
  const suggestion = engine.suggestReply({
    productNames: ["LS-UH319-W"],
    messages: [{ role: "customer", text: "해상도가 너무 낮아요. 배율도 너무 큽니다." }]
  });

  assert.equal(suggestion.generationSource, "retrieval_contextual");
  assert.match(suggestion.replyText, /해상도/);
  assert.match(suggestion.replyText, /배율/);
  assert.doesNotMatch(suggestion.replyText, /화면이 나오지 않/);
});
...
test("제품 스펙 카탈로그를 근거와 LLM 컨텍스트에 함께 포함한다", async () => {
  ...
  const llmClient = {
    ...
    async generateReplyEnhancement(context) {
      ...
      return {
        replyText:
          "LS-UH319-W 제품 특성상 드라이버 설치와 HDMI 입력 소스 확인을 먼저 부탁드립니다.",
        needsReview: false,
        reasoning: "동일 모델 스펙과 기존 초안을 함께 참고했습니다.",
        missingInformation: []
      };
    }
  };
  ...
  const productCatalog = {
    findBestMatch() {
      return {
        ...
        formFactor: "USB 3.0 to HDMI 컨버터",
        material: "ABS",
        usage: "USB3.0을 HDMI로 변환해 모니터 출력",
        specs: {
          INPUT: "USB 3.0",
          OUTPUT: "HDMI"
        },
        specsText: "INPUT: USB 3.0 / OUTPUT: HDMI",
        summary:
          "LS-UH319-W / 카테고리: 컨버터 / 형태: USB 3.0 to HDMI 컨버터 / 용도: USB3.0을 HDMI로 변환해 모니터 출력 / 규격: INPUT: USB 3.0 / OUTPUT: HDMI"
      };
    }
  };
  ...
  const suggestion = await engine.suggestReplyEnhanced({
    ...
    messages: [
      {
        role: "customer",
        text: "제품 연결 후 모니터 화면이 안 나오고 인식이 안 됩니다."
      }
    ]
  });

  assert.equal(suggestion.generationSource, "llm_hybrid");
  assert.equal(suggestion.productProfile.model, "LS-UH319-W");
  assert.equal(
    suggestion.evidence.some((item) => item.source === "제품 스펙"),
    true
  );
  assert.equal(capturedContext.productProfile.model, "LS-UH319-W");
  assert.equal(capturedContext.productProfile.category, "컨버터");
  assert.equal(capturedContext.productProfile.specs.INPUT, "USB 3.0");
});
